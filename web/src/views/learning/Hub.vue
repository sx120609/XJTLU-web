...
import { learningMaterialsApi, type LearningMaterialItem } from "@/api/learningMaterials";
import { servicesApi, type ServiceCard } from "@/api/services";

const router = useRouter();
const goods = ref<MarketItem[]>([]);
const topics = ref<Topic[]>([]);
const originals = ref<LearningMaterialItem[]>([]);
...
onMounted(async () => {
  const results = await Promise.allSettled([
    ...
    learningMaterialsApi.items({ page: 1, size: 5, sort: "new" }, { suppressErrorMessage: true }),
    servicesApi.list("学习"),
  ]);
