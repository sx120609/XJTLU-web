    ...
    <section class="home-section market-section">
      ...
      <div v-else-if="recommendedItems.length" class="market-grid">
        <router-link
          ...
          class="market-card"
          :class="{ 'market-card--promoted': item.promotions?.home || item.promotions?.pinned }"
...
const recommendedItems = computed<MarketItem[]>(() => {
  ...
  return [...promoted, ...naturalRecommendedItems.value.filter((item) => !seen.has(item.id))].slice(0, 8);
});
const quickActions = [
...
watch(canBrowseMarket, (enabled) => {
  ...
  else naturalRecommendedItems.value = [];
});
...
async function loadRecommendedItems() {
  ...
  try {
    const result = await marketApi.items({ page: 1, size: 24, listingType: "sell", sort: "popular" }, options);
    if (sequence !== marketLoadSequence) return;
    naturalRecommendedItems.value = result.list;
