  ...
  <div class="promotion-center">
    ...
    <section class="orders-section">
      ...
      <div class="order-list" v-loading="loading">
        <article v-for="order in orders" :key="order.id" class="cpu-card">
          ...
          <dl><div><dt>订单编号</dt><dd>{{ order.outTradeNo }}</dd></div><div><dt>推广费用</dt><dd>¥{{ order.amount }}</dd></div><div><dt>曝光</dt><dd>{{ order.impressionCount }}</dd></div><div><dt>点击</dt><dd>{{ order.clickCount }}（{{ order.ctr }}%）</dd></div><div v-if="order.verifiedAmount"><dt>人工核验</dt><dd>{{ verificationMethodLabel(order.verificationMethod) }} · ¥{{ order.verifiedAmount }}</dd></div><div v-if="order.verificationReferenceMasked"><dt>核验凭证</dt><dd>{{ order.verificationReferenceMasked }}</dd></div></dl>
          ...
          <p v-if="order.adminNote">处理说明：{{ order.adminNote }}</p>
          <p v-else-if="order.status === 'waitlisted'">目前推广服务已满，已进入候补队列；空位释放后会发送站内通知。</p>
          <p v-else-if="order.status === 'pending' && order.paymentSubmittedAt">付款确认已提交，等待管理员核验金额、流水和四位秘钥。</p>
          <p v-else-if="order.status === 'pending'">位置已保留，请在有效时间内扫码付款并回填四位秘钥。</p>
          <el-alert v-if="order.status === 'rejected' && order.paymentSubmittedAt" type="warning" :closable="false" show-icon title="该订单已确认过付款但未通过，系统不会自动退款；请私下联系管理员确认退款。" />
          <footer>
    ...
    <el-dialog v-model="orderOpen" title="创建推广订单" width="500px">
      <template v-if="selectedPlan">
        ...
        <el-alert type="warning" :closable="false" title="有空位时提交后将显示收款码和四位付款秘钥；满位时只进入候补，不会要求付款。" />
      </template>
...
async function submitOrder() {
  if (!selectedPlan.value || !orderForm.targetId) return ElMessage.warning("请选择推广对象");
  ...
  try {
    ...
    if (order.status === "waitlisted") ElMessage.warning("目前推广服务已满，申请已进入候补队列");
...
async function cancelOrder(order: PromotionOrder) { await ElMessageBox.confirm("确认取消这条推广申请？", "取消申请", { type: "warning" }); await marketApi.cancelPromotionOrder(order.id); ElMessage.success("推广申请已取消"); await load(); }
...
function targetTitle(order: PromotionOrder) { return order.marketItem?.title || order.wantedPost?.title || "推广对象已不可用"; }
function orderStatusLabel(status: PromotionOrder["status"]) { return ({ waitlisted: "候补中", pending: "待付款/核验", confirmed: "推广中", rejected: "未通过", cancelled: "已取消", expired: "已到期" } as const)[status]; }
