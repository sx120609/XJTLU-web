    ...
    <div v-else-if="items.length" class="materials-grid">
      <article v-for="item in items" :key="item.id" class="material-card" @click="openItem(item.id)">
        ...
        <div class="material-copy">
          ...
          <footer><UserAvatar :size="30" :src="item.seller.avatar" :name="item.seller.nickname" /><div><b>{{ item.seller.nickname||item.seller.username }}</b><small v-if="item.seller.major">{{ item.seller.major }}</small><span>{{ isEnglish ? "Campus publisher" : "校园资料发布者" }}</span></div><em>{{ item.favoriteCount }} {{ isEnglish ? "favorites" : "收藏" }}</em></footer>
        </div>
      </article>
    </div>
...
.material-copy footer small{color:var(--cpu-text-secondary);font-size:8px}
</style>
