  ...
  <div class="material-publish-page">
    <header class="page-head">
      <div>
        ...
        <h1>{{ editingId ? "编辑免费原创" : "发布免费原创" }}</h1>
        <p>清楚标注课程、适用阶段与资料来源，帮助同学判断内容是否适合自己。</p>
      </div>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="publish-form cpu-card" v-loading="loading">
      ...
      <section>
        <div class="section-title"><div><span>05</span><h2>资料文件与版本</h2></div><small>文件由平台安全存储，领取后才开放下载</small></div>
      ...
      <section>
        <div class="section-title"><div><span>06</span><h2>免费分享与权利确认</h2></div><small>本专区只允许免费原创或已获授权内容</small></div>
        <el-alert type="success" :closable="false" show-icon title="内容将以 0 元免费分享，领取记录仅用于访问控制与问题反馈。" />
        <el-form-item prop="profile.rightsConfirmed">
          <el-checkbox v-model="form.profile.rightsConfirmed" size="large">我确认拥有发布和免费分享该内容的合法权利，并接受平台内容规则</el-checkbox>
        </el-form-item>
      ...
      <footer class="form-actions">
        ...
        <el-button type="primary" :loading="submitting" @click="submit(false)">{{ editingId ? "保存并发布" : "发布免费原创" }}</el-button>
      </footer>
    </el-form>
...
async function submit(draft:boolean){if(submitting.value)return;normalizeCode();if(!draft){try{await formRef.value?.validate()}catch{return}if(!editingId&&!pendingFiles.value.length)return ElMessage.warning("正式发布前请上传至少一个资料文件");if(editingId&&!existingFiles.value.length&&!pendingFiles.value.length)return ElMessage.warning("正式发布前请上传至少一个资料文件")}else if(!form.title.trim()||!form.description.trim())return ElMessage.warning("草稿也需要填写标题和资料介绍");submitting.value=true;try{const payload:LearningMaterialItemInput={title:form.title.trim(),description:form.description.trim(),price:0,originalPrice:null,images:[...form.images],profile:{...form.profile,pageCount:form.profile.pageCount??null,applicableSemester:form.profile.applicableSemester||null},draft:true};let item=editingId?await learningMaterialsApi.updateItem(editingId,payload):await learningMaterialsApi.createItem(payload);await uploadPendingVersion(item.id);if(!draft)item=await learningMaterialsApi.updateItem(item.id,{...payload,draft:false,status:"active"});clearPublishDraft("learning-material",auth.user?.id);if(!draft&&item.status==="reviewing")ElMessage.warning("内容已提交人工复核，通过后会公开展示");else ElMessage.success(draft?"草稿和文件版本已保存":"免费原创已发布");await router.replace({name:draft?"market-learning-materials-edit":"market-learning-material-item",params:{id:item.id},query:draft?undefined:{published:"1"}})}finally{submitting.value=false}}
</script>
