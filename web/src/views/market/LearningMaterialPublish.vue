<template>
  <div class="material-publish-page">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="publish-form cpu-card" v-loading="loading">
      <el-alert
        class="required-field-alert"
        type="warning"
        :closable="false"
        show-icon
        :title="isEnglish ? 'Fields marked * are required. Complete every required field before submitting for review.' : '带 * 的字段为必填项，所有必填项完成后才能提交审核。'"
      />
      ...
      <section>
        <div class="section-title"><div><span>02</span><h2>{{ isEnglish ? "Course and applicability" : "课程与适用范围" }}</h2></div><small>{{ isEnglish ? "Complete every field marked * before submitting" : "提交前请完成所有带 * 的必填项" }}</small></div>
      ...
      <section>
        ...
        <div class="version-fields"><el-form-item :label="isEnglish ? 'File version label' : '文件版本标签'"><el-input v-model="versionDraft.label" maxlength="80" :placeholder="isEnglish ? 'Example: 2026 edition / v1.2' : '例如 2026版 / v1.2'" /></el-form-item></div>
    ...
    <el-dialog v-model="customTypeOpen" :title="isEnglish ? 'Create material type' : '创建资料类型'" width="430px">
      <p class="dialog-note">{{ isEnglish ? "A custom type is available immediately and stays visible only to you. It will not enter other users' type lists." : "自定义类型创建后立即可用，只对你本人可见，不会进入其他用户的类型列表。" }}</p>
...
const form=reactive({title:"",description:"",price:9.9,originalPrice:undefined as number|undefined,images:[] as string[],profile:{courseCode:"",college:"",major:"",typeId:null as number|null,applicableSemester:"",fileFormats:[] as string[],versionLabel:"",language:"",originalityStatement:"",rightsConfirmed:false}});
...
async function load(){
  ...
  try{
    ...
    if(editingId){
      ...
      if(localVersion&&typeof localVersion==="object")versionDraft.label=String(localVersion.label||"");
...
async function submit(draft:boolean){
  ...
  try{
    const payload:LearningMaterialItemInput={title:form.title.trim(),description:form.description.trim(),price:form.price,originalPrice:form.originalPrice??null,images:[...form.images],profile:{courseCode:form.profile.courseCode,college:form.profile.college,major:form.profile.major,typeId:form.profile.typeId,applicableSemester:form.profile.applicableSemester||null,fileFormats:[...form.profile.fileFormats],versionLabel:form.profile.versionLabel,language:form.profile.language,originalityStatement:form.profile.originalityStatement,rightsConfirmed:form.profile.rightsConfirmed},draft:true};
