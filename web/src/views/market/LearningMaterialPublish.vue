  ...
  try{
    ...
    if(editingId){
      ...
      if(item.material){
        ...
        existingFiles.value=(editableVersion?.files||[]).map(file=>({id:file.id,originalName:file.originalName,format:file.format,fileSize:file.fileSize}));
...
async function uploadPendingVersion(itemId:number){if(!pendingFiles.value.length)return null;fileUploading.value=true;try{const version=await learningMaterialsApi.uploadVersion(itemId,pendingFiles.value,{label:versionDraft.label||form.profile.versionLabel,releaseNotes:versionDraft.releaseNotes},{timeout:10*60*1000,onUploadProgress:(event)=>{const total=Number(event.total||0);fileUploadProgress.value=total?Math.round(Number(event.loaded||0)/total*100):0}});existingFiles.value=version.files.map(file=>({id:file.id,originalName:file.originalName,format:file.format,fileSize:file.fileSize}));pendingFiles.value=[];return version}finally{fileUploading.value=false;fileUploadProgress.value=0}}
