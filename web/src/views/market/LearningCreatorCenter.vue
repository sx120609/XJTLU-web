<template>
  <div class="publisher-page" v-loading="loading">
    ...
    <el-alert type="info" :closable="false" show-icon :title="isEnglish ? 'The platform does not collect payment. Buyers pay your QR code directly; full files unlock only after you confirm receipt.' : '平台不代收资料款。买家直接向你的收款码付款；你核对到账后确认，系统才会解锁完整资料。'" />
    ...
    <section class="cpu-card collection-card">
      <div class="section-head">
        ...
        <el-button type="primary" :disabled="!activePublisher" @click="methodDialog = true">{{ isEnglish ? "Add collection QR" : "添加收款码" }}</el-button>
      </div>
