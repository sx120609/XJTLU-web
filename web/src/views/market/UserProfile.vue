  ...
  <div class="profile-page" v-loading="loading">
    <template v-if="profile">
      <header class="profile-hero cpu-card">
        ...
        <div class="hero-actions"><el-button v-if="canReport" type="danger" plain @click="reportOpen = true">举报用户</el-button><el-button @click="$router.push('/market')">返回市集</el-button></div>
      </header>
      <section class="stats cpu-card"><div><strong>{{ trust?.score ?? '—' }}</strong><span>信誉值</span></div><div><strong>{{ profile.stats.rating.toFixed(1) }}</strong><span>交易评分</span></div><div><strong>{{ trust?.completedTradeCount ?? profile.stats.completedTrades }}</strong><span>成交笔数</span></div><div><strong>{{ trust?.completionRate ?? 0 }}%</strong><span>成交率</span></div><div><strong>{{ trust?.positiveRate ?? profile.stats.positiveRate }}%</strong><span>好评率</span></div><div><strong>{{ profile.stats.listingCount }}</strong><span>公开在售</span></div></section>
      ...
      <el-alert type="info" :closable="false" show-icon title="个人主页不会公开学号、登录名或联系方式。对在售商品有意向时可直接发起站内私聊。" />
    </template>
