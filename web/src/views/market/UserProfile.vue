  ...
  <div class="profile-page" v-loading="loading">
    <template v-if="profile">
      <header class="profile-hero cpu-card">
        ...
        <div><span>KAOPU · SELLER</span><h1>{{ profile.user.nickname || (isEnglish ? 'Campus user' : '校园用户') }}</h1><p>{{ trust?.identity.label || (profile.user.studentSso ? (isEnglish ? 'XJTLU campus identity verified' : 'XJTLU 校园身份已认证') : (isEnglish ? 'Campus platform user' : '校园平台用户')) }} · {{ isEnglish ? "Joined" : "加入于" }} {{ joinDate }}</p></div>
