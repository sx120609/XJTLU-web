  ...
  <div class="profile-page" v-loading="loading">
    <template v-if="profile">
      <header class="profile-hero cpu-card">
        ...
        <div><span>KAOPU · SELLER</span><h1>{{ profile.user.nickname || '校园用户' }}</h1><p>{{ trust?.identity.label || (profile.user.studentSso ? 'XJTLU 校园身份已认证' : '校园平台用户') }} · 加入于 {{ joinDate }}</p></div>
        <div class="hero-actions"><el-button v-if="profile.merchant" type="primary" plain @click="$router.push(`/market/merchant/${profile.merchant.slug}`)">合作商户主页</el-button><el-button v-if="canReport" type="danger" plain @click="reportOpen = true">举报用户</el-button><el-button @click="$router.push('/market')">返回市集</el-button></div>
      </header>
      <section class="stats cpu-card"><div><strong>{{ trust?.score ?? '—' }}</strong><span>{{ trust?.label || '信用分' }}</span></div><div><strong>{{ profile.stats.rating.toFixed(1) }}</strong><span>交易评分</span></div><div><strong>{{ trust?.completedTradeCount ?? profile.stats.completedTrades }}</strong><span>完成交易</span></div><div><strong>{{ trust?.positiveRate ?? profile.stats.positiveRate }}%</strong><span>好评率</span></div><div><strong>{{ profile.stats.listingCount }}</strong><span>公开在售</span></div><div><strong>{{ trust?.noShowCount ?? profile.stats.noShowCount }}</strong><span>爽约记录</span></div></section>
      <section><div class="section-head"><div><span>ACTIVE LISTINGS</span><h2>在售物品</h2></div><small>仅展示公开、有效的商品</small></div><div v-if="profile.recentItems.length" class="goods-grid"><article v-for="item in profile.recentItems" :key="item.id" @click="$router.push(`/market/item/${item.id}`)"><div><img v-if="item.cover" :src="item.cover" :alt="item.title" /><span v-else>📦</span></div><h3>{{ item.title }}</h3><p><strong>¥{{ item.price }}</strong><span>{{ item.campus || '校内面交' }}</span></p></article></div><el-empty v-else description="该用户暂时没有公开在售物品" /></section>
      <el-alert type="info" :closable="false" show-icon title="个人主页不会公开学号、登录名或联系方式。只有交易意向被接受后，双方才可进入交易会话。" />
    </template>
    <el-empty v-else-if="!loading" description="用户交易主页不存在"><el-button @click="$router.push('/market')">返回市集</el-button></el-empty>
    <el-dialog v-model="reportOpen" title="举报该用户" width="440px"><el-form label-position="top"><el-form-item label="举报原因"><el-input v-model="report.reason" maxlength="80" placeholder="例如：疑似欺诈、骚扰或违规交易" /></el-form-item><el-form-item label="补充说明"><el-input v-model="report.detail" type="textarea" :rows="4" maxlength="1000" show-word-limit /></el-form-item></el-form><template #footer><el-button @click="reportOpen = false">取消</el-button><el-button type="danger" :loading="reporting" @click="submitReport">提交举报</el-button></template></el-dialog>
  </div>
</template>
...
const joinDate = computed(() => profile.value?.user.createdAt ? new Date(profile.value.user.createdAt).toLocaleDateString("zh-CN", { year: "numeric", month: "long" }) : "近期");
onMounted(load);
...
async function submitReport() { if (!profile.value || reporting.value) return; if (report.reason.trim().length < 2) return void ElMessage.warning("请填写举报原因"); reporting.value = true; try { await marketApi.reportUser(profile.value.user.id, report); reportOpen.value = false; report.reason = ""; report.detail = ""; ElMessage.success("举报已提交，管理员会尽快核查"); } finally { reporting.value = false; } }
</script>
