<template>
  <div class="wanted-page">
    ...
    <section class="wanted-content" v-loading="loading">
      <div class="content-head"><div><h2>最新求购</h2><span>{{ total }} 条有效需求</span></div><small>联系方式默认不公开 · 接受响应后进入校内预约</small></div>
      ...
      <div v-else-if="posts.length" class="wanted-grid">
        <article v-for="post in posts" :key="post.id" :class="{ urgent: post.promotion.urgent }" @click="openPost(post)">
          <header><span>{{ categoryIcon(post.category) }}</span><em>{{ statusLabel(post.status) }}</em><time>{{ fmtRelative(post.createdAt) }}</time></header>
...
onMounted(async () => {
  try { categories.value = (await marketApi.meta({ suppressErrorMessage: true })).categories; } catch { categories.value = []; }
  await load();
});
