<template>
  <div class="wanted-page">
    ...
    <section class="wanted-content" v-loading="loading">
      <div class="content-head"><div><h2>{{ filters.sort === 'popular' ? '热门求购' : '最新求购' }}</h2><span>{{ total }} 条有效需求</span></div><small>联系方式默认不公开 · 接受响应后进入校内预约</small></div>
      ...
      <div v-else-if="posts.length" class="wanted-grid">
        <article v-for="post in posts" :key="post.id" :class="{ urgent: post.promotion.urgent }" @click="openPost(post)">
          ...
          <div v-if="filters.sort === 'popular' && post.hotReasons.length" class="hot-reasons"><span>热门原因</span>{{ post.hotReasons.join(' · ') }}</div>
