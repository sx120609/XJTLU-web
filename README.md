...
> 只面向 XJTLUer 的可信校园交易与生活空间

靠浦是一个面向西交利物浦大学学生的可信校园交易平台：校内买卖、求购、交流和靠谱信息，一处完成。
...
仓库地址：[https://github.com/sx120609/XJTLU-web](https://github.com/sx120609/XJTLU-web)
...
| 模块 | 典型路径 | 访问要求 | 说明 |
|---|---|---|---|
| 首页 / 搜索 | `/home` `/search` | 公开 | 首页只展示推荐好物、热议求购和三个发布捷径，不展示校园资源或校园公告 |
| 广场入口 / 板块 / 帖子 | `/square` `/forum/b/:slug` `/forum/topic/:id` | 公开或受社区权限控制 | `/forum` 保留为兼容跳转；具体社区内容仍会受服务端权限与功能开关拦截 |
...
- 校园市集：支持结构化实体商品、独立求购、购买意向、预订、线下交付、双方确认、真实评价、举报和分享；学生商品款由买家直接支付给卖家，平台不代收、不抽佣。
- 广场与学习中心：广场保留 12 个频道，按综合讨论、学习交流、生活社交分组；学习好物复用实体市集，学生原创内容首期只允许免费分享并接受审核。
- 推广与合作商户：支持商品置顶、求购加急、首页推广和审核后的商户主页；商业内容明确标注，推广费订单首期由管理员人工确认。
...
该命令会执行活动迁移并写入开发种子数据。生产环境只执行迁移，不运行 seed：
