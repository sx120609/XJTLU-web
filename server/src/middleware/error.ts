...
export function errorHandler(err: unknown, req: Request, res: Response, _next: NextFunction) {
  ...
  const message = (value: string) => localizeApiMessage(value, locale);
  ...
  if (
    ...
    return res.status(413).json({
      code: 4013,
      data: null,
      message: message("上传内容过大，请压缩图片或更换更小的文件后重试"),
    });
  }
  if (err instanceof HttpError) {
    return res.status(err.status).json({ code: err.code, data: null, message: message(err.message) });
  }
  if (err instanceof ZodError) {
    const msg = err.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join("; ");
    return res.status(400).json({ code: 4000, data: null, message: message("参数错误：" + msg) });
  }
  ...
  if (anyErr?.name === "PrismaClientValidationError") {
    return res.status(400).json({ code: 4000, data: null, message: message("请求参数格式不正确") });
  }
  if (anyErr?.code === "P2025") {
    return res.status(404).json({ code: 4004, data: null, message: message("资源不存在或已被移除") });
  }
  if (["P2002", "P2003", "P2034"].includes(anyErr?.code || "")) {
    return res.status(409).json({ code: 4009, data: null, message: message("数据已发生变化，请刷新后重试") });
  }
  ...
  return res.status(500).json({ code: 5000, data: null, message: message("服务器内部错误，请稍后再试") });
}
